    ...
    <List.Root>    
        ...
        {data?.results.map(genre => <List.Item key={genre.id} paddingY='5px'>
            <HStack>        
                ...
                <Button whiteSpace='normal' textAlign="left" textWrap= "wrap"  fontWeight={selectedGenre?.id === genre.id ? 'bold' : 'normal'} onClick={() => onSelectGenre(genre)} fontSize='md' variant='plain'>  {genre.name}</Button>
            </HStack>
        </List.Item>)}
    </List.Root>
</>
